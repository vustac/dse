    ...
    switch(select) {
      ...
      case "CONTROLS":
        ...
        printParagraph("This panel provides the user the ability to select any of the instrumented methods " +
                       ...
                       "a ^^bcextractor/classes^^ subfolder of the path where the project jar file resides) and will then " +
                       "run ^^javap^^ on the class to generate the bytecode file (which will be placed in the " +
                       "^^bcextractor/javap^^ subfolder). The bytecode information for the specified file in that class " +
                       ...
                       "Note that the class extraction and javap conversion is performed by a separate program " +
                       "running as a server receiving connand requests on port 6000. If this program is not running " +
                       "the bytecode information cannot be displayed. The ^^classes^^ and ^^javap^^ directories can be set to " +
                       "clear out all files based on the System Parameter settings that specify whether to purge these when " +
                       ...
                       "between runs, you should at least delete them every time you load a jar file.");
