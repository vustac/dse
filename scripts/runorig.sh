...
function add_curdir_to_classpath
{
    ...
    while read -r jarfile; do
        ...
        # ignore the original and danalyzed version of the jar file
...
# setup classpath
CLASSPATH=""
add_file_to_classpath "${PROJJAR}.jar"
add_curdir_to_classpath
add_dir_to_classpath "lib"
add_dir_to_classpath "libs"

# run the un-instrumented code
if [[ ${TESTMODE} -eq 0 ]]; then
    java -cp ${CLASSPATH} ${MAINCLASS} ${ARGLIST}
else
    echo "java -cp ${CLASSPATH} ${MAINCLASS} ${ARGLIST}"
fi
